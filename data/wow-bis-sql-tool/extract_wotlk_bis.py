...
def render_sql(
    ...
    # Use the role-specific icon when a spec has a distinct talent base
    # (Druid Feral Bear/Tank -> bear-form icon, not the Cat default).
    icon = SPEC_ICONS.get((player_class, talent_spec_base or spec_base),
                          SPEC_ICONS.get((player_class, spec_base), "inv_misc_questionmark"))
    spec_label = suffix_to_label(suffix)
    gossip_text = f"|cff00ff00|TInterface\\\\icons\\\\{icon}:30|t|r Use {spec_base} {spec_label}"
    # talent_spec_base lets a shared-tree spec point at its own talent set
    # (e.g. Druid Feral Bear/Tank -> FeralTank while gear/name stays Feral).
    override_base = talent_spec_base or spec_base
    talent_spec = f"{override_base}{talent_override_suffix}"
    # Glyphs use their own override set (keyed by GlyphProperties IDs, generated by
    # extract_wotlk_glyphs.py); honour the same distinct base (e.g. FeralTank).
    glyph_spec = f"{override_base}{glyph_override_suffix}" if glyph_override_suffix else talent_spec
