...
def main() -> None:
    ...
    parser.add_argument(
        "--glyph-suffix", default="80PvE",
        help="glyphOverride suffix (default: 80PvE — generated by extract_wotlk_glyphs.py)",
    )
    ...
    for idx, (cls, spec, role, url) in enumerate(entries):
        ...
        try:
            ...
            # Druid Feral Bear (tank) needs its own talent set, distinct from Cat (dps),
            # even though both share the "Feral" spec base for gear/naming.
            talent_spec_base = ("FeralTank" if player_class == "Druid" and spec_base == "Feral"
                                and role == "tank" else None)
            sql = mod.render_sql(
